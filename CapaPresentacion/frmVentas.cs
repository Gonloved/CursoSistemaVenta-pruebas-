...
namespace CapaPresentacion
{
    public partial class frmVentas : Form
    {
        ...
        private void calcularcambio()
        {
            ...
            {
                // Si el pago es menor al total, mostrar 0.00 (sin cambio negativo)
                if (pagacon < total)
                {
                    txtcambio.Text = "0.00";
                }
            ...
            {
                // Si el valor no es válido, limpiar cambio
                txtcambio.Text = "0.00";
            }
        }
        ...
        private void btncrearventa_Click(object sender, EventArgs e)
        {
            ...
            // VALIDACIÓN: Monto de pago debe ser mayor o igual al monto total
            decimal montoTotal = 0;
            decimal montoPago = 0;

            if (!decimal.TryParse(txttotalpagar.Text, out montoTotal))
            {
                MessageBox.Show("Monto total no válido", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (!decimal.TryParse(txtpagocon.Text, out montoPago))
            {
                MessageBox.Show("Monto de pago no válido", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtpagocon.Focus();
                return;
            }
            ...
            {
                MessageBox.Show("El monto de pago no puede ser menor al monto total.", "Validación de Pago", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtpagocon.Focus();
                return;
            }
